...
app.controller('goodsController', function ($scope, $controller,$location,goodsService,
    ...
    $scope.uploadFile = function () {
        uploadService.uploadFile().success(
            function (response) {
                if (response.success) {
                    $scope.image_entity.url = response.message;//如果上传成功，获取文件地址
                } else {
                    alert(response.message);
                }
            }
    ...
    $scope.$watch("entity.goods.category1Id", function (newValue, oldValue) {
        ...
        itemCatService.findByParentId(newValue).success(
            function (response) {
                ...
                $scope.itemCat3List = {};//清空3级列表
                $scope.entity.goods.typeTemplateId = undefined;//清空模板id
            }
        )
    })
    //读取三级分类
    ...
    $scope.updateSpecAttribute = function ($event, name, value) {
        ...
        if (object != null) {
            //如果集合中已经存在这个属性
            if ($event.target.checked) {//如果是选中，就添加元素
